...
function getRandomString(type: keyof typeof CHAR_DATA, length: number): string {
  let data = "";
  for (let i = 0; i < length; i++) {
    const randomIndex = getRandomInt(CHAR_DATA[type].length);
    data += CHAR_DATA[type].charAt(randomIndex);
  }
  return data;
}
...
function createRedBodyData(): string[][] {
  let csvData: string[][] = [HEADER];
  csvData.push(Array(HEADER.length).fill(""));
  const max = [8, 20, 20, 20, 20, 20, 20, 50];
  // 境界内最大値
  Object.keys(CHAR_DATA).forEach((key) => {
    csvData.push(max.map((max) => getRandomString(key as keyof typeof CHAR_DATA, max)));
  });
  // 境界外最小値
  Object.keys(CHAR_DATA).forEach((key) => {
    csvData.push(max.map((max) => getRandomString(key as keyof typeof CHAR_DATA, max+1)));
  });
  return csvData;
}
