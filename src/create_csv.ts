...
const CLASS_DATA = ["役員", "正社員", "派遣社員", "アルバイト", "パート"];
const GROUP_DATA = [
  "本社",
  "神奈川支社",
  "埼玉支社",
...
const CHAR_DATA = {
  FullWidthNumber: "０１２３４５６７８９",
  HalfWidthNumber: "0123456789",
  Hiragana:
...
function createRandomData(type: StringType, length: number): string {
  let data = "";
  const value = (): string => {
    switch (type) {
      case StringType.FullWidthNumber:
        return CHAR_DATA.FullWidthNumber;
      case StringType.HalfWidthNumber:
        return CHAR_DATA.HalfWidthNumber;
      case StringType.Hiragana:
        return CHAR_DATA.Hiragana;
      case StringType.Katakana:
        return CHAR_DATA.Katakana;
      case StringType.Kanji:
        return CHAR_DATA.Kanji;
      case StringType.Alphabet:
        return CHAR_DATA.Alphabet;
      case StringType.Symbol:
        return CHAR_DATA.Symbol;
    }
  };
  for (let i = 0; i < length; i++) {
    const randomIndex = getRandomInt(value().length);
    data += value().charAt(randomIndex);
  }
  return data;
}

async function createGreenData(dataCount: number): Promise<string[][]> {
  const nameSets = await getName(dataCount);
  let csvData: string[][] = [HEADER];
  for (let row = 0; row < dataCount; row++) {
    let employeeCode = (row + 1).toString();
    csvData.push([
      employeeCode.padStart(8, "0"),
      nameSets[row][0][0],
      nameSets[row][0][1],
      hiraToKana(nameSets[row][1][0]),
      hiraToKana(nameSets[row][1][1]),
      POSITION_DATA[row % POSITION_DATA.length],
      CLASS_DATA[row % CLASS_DATA.length],
      GROUP_DATA[row % GROUP_DATA.length]
    ]);
  }
  return csvData;
}

function createRedBodyData(): string[][] {
  let csvData: string[][] = [HEADER];
  Object.entries(StringType).forEach(([key, value]) => {
